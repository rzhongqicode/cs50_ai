...
class NimAI:
    ...
    def get_q_value(self, state, action):
        ...
        """
        state_acition_pair = (tuple(state), action)
        if state_acition_pair in self.q:
            return self.q[state_acition_pair]
        else:
            return 0
    ...
    def choose_action(self, state, epsilon=True):
        ...
        if not epsilon:
            ...
            if not possible_actions:
                return None
            action = max(
                possible_actions, key=lambda action: self.get_q_value(state, action)
            )
            return action
        else:
